"""ChainPatternAtom — rule atoms for pattern matching in Chains.

Each atom is immutable, hashable, and produces a list of possible
consumed-element-counts via :meth:`ChainPatternAtom.match_lengths`
(shortest first = non-greedy).

Available atoms (created via :class:`ChainPatternAtom` static methods):
...
def _rex_factory(pattern: str) -> ChainPatternAtom:
    ...
    >>> ChainPatternAtom.rex(r'\\d+')     # matches '123'
    """
    return _Rex(pattern)


def _enum_factory(*alternatives) -> ChainPatternAtom:
...
def _un_factory(value: str) -> ChainPatternAtom:
    ...
    Examples
    --------
    >>> ChainPatternAtom.un('admin')  # matches anything except 'admin'
    """
    return _Un(value)
...
# Attach factory methods to ChainPatternAtom
ChainPatternAtom.any = staticmethod(_any_factory)
